...
class ProductRepoImplementation extends ProductRepository {
  async addProduct(product: ProductModel): Promise<ApiResponse<ProductModel>> {
    console.log(
      "🚀 ~ file: productRepoImplementation.ts:11 ~ ProductRepoImplementation ~ addProduct ~ product:",
      typeof product.category,
    )

    product.category = new Types.ObjectId(product.category)
    console.log("🚀 ~ ProductRepoImplementation ~ addProduct ~ product.category:", typeof product.category)

    try {
      const response = await ProductModelScheme.create(product)
      console.log("🚀 ~ ProductRepoImplementation ~ addProduct ~ response:", response)

      const populatedResponse = await ProductModelScheme.findById(response._id)
        .populate("category")
        .populate("productImages")
        .exec()

      console.log(
        "🚀 ~ file: productRepoImplementation.ts:14 ~ ProductRepoImplementation ~ addProduct ~ populatedResponse:",
        populatedResponse,
      )
  ...
  async getProduct({ productId }: { productId: string }): Promise<ApiResponse<ProductModel>> {
    try {
      ...
      const response = await ProductModelScheme.findById({ _id: id })
        ...
        .populate({ path: "productImages", select: "-__v" })
        .select("-__v")
        .exec()
  ...
  async getAllProducts(): Promise<ApiResponse<ProductModel[]>> {
    try {
      const response: ProductModel[] = await ProductModelScheme.find()
        .populate({
          path: "category",
          select: "-__v", // Exclude __v from category
        })
        .populate({
          path: "productImages",
          select: "-__v", // Exclude __v from productImages
        })
        .select("-__v") // Exclude __v from the main ProductModel
        .exec()
